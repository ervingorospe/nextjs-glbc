...
// components
import { Title, Body, Button, VideoHolder, ImageHolder, AlertMessage } from '@/components/shared'
// constants
import { defaultHeroStyles } from '@/app/constants'

const defaults = { ...defaultHeroStyles, width: 'max-w-screen-xl',textAlign: 'text-center' }

export async function HeroBanner({ data, sectionCount, searchParams }) {
  const { fields } = data
  ...
  return (
    <Section className="relative w-full" bg={bg}>
      ...
      <Container className="relative" width={width} margin="hero">
        <div className="grid justifty-center">
          <div className="">
            {
              (searchParams.purchase && searchParams.purchase > 0) && (
                <AlertMessage/>
              )
            }
            {/* title */}
            ...
            {/* subtitle */}
            <h2 className="mt-1 prose prose-lg prose-secondary-invert md:prose-xl text-center mx-auto">{_.get(fields, 'subtitle')}</h2>

            {/* body */}
            <Body
              body={_.get(fields, 'body')}
              size={bodySize}
              className={`${_.get(fields, 'bodyClass') ?? _.get(fields, 'bodyClass')} text-center mx-auto`}
            />
            ...
            <Body
              ...
              size={extraBodySize}
              className={`mt-1 ${_.get(fields, 'extraBodyClass') ?? _.get(fields, 'extraBodyClass')} text-center mx-auto`}
            />
