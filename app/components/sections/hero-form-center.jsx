...
const defaults = { ...defaultHeroStyles, textAlign: 'text-center' }

export function HeroFormCenter({ data, sectionCount }) {
  const { fields } = data
  ...
  const buttonStyle2 = _.get(fields, 'buttonStyle-2') !== 'default' ? _.get(fields, 'buttonStyle-2') : defaults.buttonStyle2
  const buttonStyle = _.get(fields, 'buttonStyle') !== 'default' ? _.get(fields, 'buttonStyle') : defaults.buttonStyle
    ...
    <Section className="relative" bg={bg}>
      ...
      <Container className="relative z-1" width={width} margin="hero">
        <div className="grid gap-8 justify-center">
          <Motion>
...

            {
              (_.get(fields, 'button') || _.get(fields, 'button-2')) && (
                <Motion className="mt-8 justify-center grid space-y-2 sm:space-y-0 space-x-0 sm:flex sm:space-x-4">
                  {
                    _.get(fields, 'button') && (
                      <MotionVariant variants={fadeInFromBottom}>
                        <Button
          ...
          <FormWufoo data={data} className="relative mx-auto max-w-2xl bg-gray-50 rounded-lg"/>
        </div>
      </Container>
