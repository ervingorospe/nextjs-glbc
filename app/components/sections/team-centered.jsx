...
const defaults = { ...defaultSectionStyles, width: "max-w-screen-lg", textAlign: 'text-center' }
        ...
        <Motion>
...

          {
            subCollections.length > 0 && (
              <div className="mt-8 grid md:grid-cols-2 justify-center gap-x-8 gap-y-16">
                {
                  subCollections?.map(team => (
                    <MotionVariant className="grid justify-center text-center" variants={fadeInFromBottom} key={team.id}>
                      <ImageHolder
                        image={_.get(team, 'fields.image')} 
                        className={{
                          figure: "aspect-[4/5] w-52 flex-none rounded-lg object-cover",
                          image: "aspect-[4/5] w-52 flex-none rounded-lg object-cover shadow-md"
                        }}
                      />

                      <h2 className="mt-4 font-heading text-lg text-gray-700 font-bold">{team.name}</h2>
